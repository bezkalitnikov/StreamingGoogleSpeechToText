...
// https://github.com/awslabs/aws-lex-browser-audio-capture/blob/master/lib/recorder.js

(function ($) {

  siberia.ensureObject('siberia.speechToText');

  var workerUrl = $('#SpeechToText').attr('data-speech-to-text-worker-url');
   ...
   */
  var recorder = function (source, silenceDetectionConfig, recordingModeConfig) {
    ...
    recordingModeConfig.stream = recordingModeConfig.hasOwnProperty('stream') ? recordingModeConfig.stream : false;
    recordingModeConfig.streamToBase64String = recordingModeConfig.hasOwnProperty('streamToBase64String') ? recordingModeConfig.streamToBase64String : false;
    recordingModeConfig.streamSampleRate = recordingModeConfig.hasOwnProperty('streamSampleRate') ? recordingModeConfig.streamSampleRate : 16000;
    recordingModeConfig.file = recordingModeConfig.hasOwnProperty('file') ? recordingModeConfig.file : true;

    var recording = false,
      currCallback, chunkCallback, start, silenceCallback, visualizationCallback;

    // Create a ScriptProcessorNode with a bufferSize of 16384 and a single input and output channel
    var node = source.context.createScriptProcessor(16384, 1, 1);

    worker.onmessage = function (message) {
      switch(message.data.type) {
        case 'file':
          currCallback(message.data.payload);
          break;
        case 'chunk':
          chunkCallback(message.data.payload);
          break;
      }
    };
     ...
     * @param {?onDataAvailableCallback} onDataAvailable - Can be used to handle next available audio data.
     */
    var record = function (onSilence, visualizer, onDataAvailable) {
      silenceCallback = onSilence;
      visualizationCallback = visualizer;
      chunkCallback = onDataAvailable;
      start = Date.now();
      recording = true;
    };
   ...
   */
  siberia.speechToText.audioRecorder = function () {
    ...
    var createRecorder = function (silenceDetectionConfig, recordingModeConfig) {
      return recorder(audio_context.createMediaStreamSource(audio_stream), silenceDetectionConfig, recordingModeConfig);
    };
