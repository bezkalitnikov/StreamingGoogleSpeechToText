...
// https://github.com/awslabs/aws-lex-browser-audio-capture/blob/master/lib/control.js
...
(function ($) {
   ...
   * is supported.
   */
  siberia.speechToText.audioControl = function (options) {
    options = options || {};
     ...
     * @param {recordingModeConfig} - Specify custom recording mode values.
     * @param {onDataAvailable} - Can be used to handle available audio data.
     * @throws {Error} If audio is not supported.
     */
    var startRecording = function (onSilence, visualizer, onDataAvailable, silenceDetectionConfig, recordingModeConfig) {
      onSilence = onSilence || function () { /* no op */
      };
      visualizer = visualizer || function () { /* no op */
      };
      onDataAvailable = onDataAvailable || function() { /* no op */
      };
      audioSupported = audioSupported !== false;
      if (!audioSupported) {
        throw new Error(UNSUPPORTED);
      }
      recorder = audioRecorder.createRecorder(silenceDetectionConfig, recordingModeConfig);
      recorder.record(onSilence, visualizer, onDataAvailable);
    };
    ...
    var supportsAudio = function (callback) {
      ...
      if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        audioRecorder = siberia.speechToText.audioRecorder();
